...
void CarouselImageWindow::onImageChangeTimeout()
{
    ...
    // 动画类重新开始; 
   // connect(m_opacityAnimation, SIGNAL(finished()), this, SLOT(onAnimationFinished()));
  //  QTimer::singleShot(100, m_opacityAnimation, SLOT(start()));

    m_opacityAnimation->start();
    emit currentImageChanged(getCurrentImageIndex());
}

void CarouselImageWindow::onAnimationFinished()
{
    update();
}
...
void CarouselImageWindow::paintEvent(QPaintEvent* event)
{
    ...
    else if (m_imageFileNameList.count() > 1)
    {
        ...
        painter.drawPixmap(imageRect, m_nextPixmap.scaled(imageRect.size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
        painter.setOpacity(imageOpacity);
        // painter.drawPixmap(imageRect, m_currentPixmap.scaled(imageRect.size()));
        painter.drawPixmap(imageRect, m_currentPixmap.scaled(imageRect.size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void CarouselImageWindow::onImageSwitchButtonClicked(int buttonId)
{
    ...
    m_imageChangeTimer.start(3000);
    update();
}

int CarouselImageWindow::getCurrentImageIndex()
{
    return m_currentDrawImageIndx;
}
